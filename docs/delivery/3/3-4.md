...
### Acceptance Criteria
- [ ] All 4 playlist endpoints implemented
- [ ] Request/response DTOs defined
- [ ] UUID validation on all ID parameters
- [ ] Playlist service integration working
...
- [ ] Position reordering verified

### Definition of Done
- All acceptance criteria met
- Integration tests pass
- Code follows existing API patterns
- No linter errors
- Transaction handling verified
- Consistent with channel API structure

## Files Modified

### New Files Created
- None if extending `channel.go`, or:
- `api/internal/api/playlist.go` - Playlist API handlers (if separate)

### Files Modified
- `api/internal/api/channel.go` - Add playlist endpoints (or reference to playlist handler)
- `api/internal/server/server.go` - Register playlist routes
- `api/test/integration/channel_api_test.go` - Add playlist endpoint tests

### API Specifications Updated
- `docs/api-specs/channels/channels-api.md` - Document all playlist endpoints
