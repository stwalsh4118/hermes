...
| 2025-01-XX XX:XX:XX | Status Change | InProgress | Done | Implementation completed with sliding window logic, mediaSequence tracking, and tests | AI_Agent |
...
### Acceptance Criteria
- [x] AddSegment adds segments correctly
- [x] Sliding window prunes old segments when windowSize exceeded
- [x] Media sequence increments correctly when segments pruned
- [x] Max duration updates correctly
- [x] Discontinuity flag handled correctly
- [x] VOD mode (windowSize=0) never prunes segments
- [x] Input validation works (empty URI, invalid duration)
- [x] Thread-safe under concurrent AddSegment calls
- [x] Pruned segment URIs returned correctly
- [x] Observability logging includes all relevant metrics

## Files Modified

### Files Modified
- `api/internal/streaming/playlist/manager.go` — Implemented AddSegment() with sliding window logic
- `api/internal/streaming/playlist/manager_test.go` — Updated tests and added new test cases
- `api/internal/streaming/manager.go` — Updated AddSegment() call to handle new return signature
