...
### Acceptance Criteria
- [ ] StartStream creates new stream or returns existing
- [ ] RegisterClient increments client count and starts stream if needed
- [ ] UnregisterClient decrements count and triggers grace period at zero
- [ ] StopStream terminates FFmpeg and cleans up resources
- [ ] Multiple clients watching same channel share one stream
- [ ] Grace period (30s default) prevents premature termination
- [ ] Cleanup goroutine stops idle streams after grace period
- [ ] FFmpeg process monitoring detects crashes
- [ ] Segment directories are created and cleaned up
- [ ] Graceful shutdown stops all streams cleanly
